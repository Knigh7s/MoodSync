...
public class ColorThief {
    ...
    public static int[][] getPalette(
            ...
            int quality,
            boolean ignoreWhite) {
        CMap cmap = getColorMap(sourceImage, region, colorCount, quality, ignoreWhite);
        if (cmap == null) {
            return null;
        }
        return cmap.palette();
    ...
    public static CMap getColorMap(Bitmap sourceImage, Rect region, int colorCount) {
        return getColorMap(sourceImage, region, colorCount, DEFAULT_QUALITY, DEFAULT_IGNORE_WHITE);
    }
    ...
    public static CMap getColorMap(
        ...
        pixelArray = getPixelsFast(sourceImage, region, quality, ignoreWhite);
            ...
            for (int row = 0; row < height; row++){
                ...
                if (!rowIsBlack){
                    for (int i =0; i<rowPixelsUsed; i++) {
                        pixelArray[numUsedPixels] = rowPixelArray[i];//new int[]{rowPixelArray[i][0], rowPixelArray[i][1], rowPixelArray[i][2]};
                        numUsedPixels++;
                    }
                }
            }
